...
Config {
    ...
    lowerOnStart = True,
    commands = [
        Run Weather "KPDX" ["-t","<tempF>F <skyCondition>","-L","64","-H","77","-n","#CEFFAC","-h","#FFB6B0","-l","#96CBFE"] 36000,
    ...
    sepChar = "%",
    alignSep = "}{",
    template = "%StdinReader% }{ %multicpu%   %memory%   %eth0%   <fc=#FFFFCC>%date%</fc>   %KPDX%"
}
